...
import { signMessage } from "utils/utils.web3";
...
export default class KeyringService extends BaseService<Events> {
  ...
  async personalSign({
    ...
    try {
      // const { dAppNetwork: network, dAppConnectAddress: account } = (
      //   await getStateFromStorage()
      // ).dappInfo;
      // const node =
      //   NETWORKCHAIN[Number(network) as keyof typeof NETWORKCHAIN][NODE_URL];
      ...
      // const hashedPassword: any = state.app.hashedPassword;
      // const privateKey = decryptMessage(
      //   accounts[`${account}`].secret,
      //   hashedPassword
      // );
      // const res = web3.eth.accounts.sign(signingData, privateKey);
      // const signature = res.signature;

      // return signature;
      // const account = await getStateValue("dappData", "currentAddress");

      const res = await signMessage(signingData);

      return res;
    } catch (err) {
      // console.log("Signing data failed personalSign", err);
      throw new Error("Signing data failed personalSign");
    }
  }
