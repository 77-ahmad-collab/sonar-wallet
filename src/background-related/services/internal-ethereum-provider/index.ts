...
import { internalProviderPort } from "store/utils/contract-utils";
...
import { ISignData } from "interfaces";
import { dispatchWithEmitter } from "utils/utils.storage";
...
export type DAppRequestEvent<T, E> = {
  payload: T;
...
type Events = ServiceLifecycleEvents & {
  ...
  setSignedDataEvent: DAppRequestEvent<ISignData, void>;
  // connect
  // disconnet
  // account change
  // networkchange
...
export default class InternalEthereumProviderService extends BaseService<Events> {
  ...
  async routeSafeRPCRequest(
    ...
    switch (method) {
      ...
      case "web3_sha3": {
        const res = await this.chainService.send(
          method,
          params,
      ...
      case DAPPEVENTS.sendTransaction:
        return this.signTransaction(
          ...
          // console.log("broadcastSignedTransaction internal eth provider");
  ...
  private async signTypedData(params: SignTypedDataRequest) {
    return new Promise<string>((resolve, reject) => {
      // localStorage.setItem("signTypedData", JSON.stringify(params)); // migrated
  ...
  private async signData(
    ...
    const activeNetwork = await this.getActiveOrDefaultNetwork(origin);
    await dispatchWithEmitter(this.emitter, "setSignedDataEvent", {
      signingData: data,
      messageType: type,
